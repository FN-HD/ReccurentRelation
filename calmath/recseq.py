    ...
    * Actualize +, -, and scalar. 
    ...
    def __init__(self, 
        ...
        if len(initial_values) < len(reccurent_relation):
            raise TypeError('init_value is too short.')
    ...
    def __str__(self):
        degree = len(self._reccurent_relations)
        self_str = f'Reccurent Relations \n' f'X**{degree}'
                        
        for i, c in enumerate(self._reccurent_relations):
            self_str = self_str + f' + {str(c)}X**{degree - i}'
        
        self_str = self_str + '\n' + 'initial values = ('
                        
        for v in self._inital_values:
